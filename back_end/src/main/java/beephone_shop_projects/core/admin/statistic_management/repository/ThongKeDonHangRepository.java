...
public interface ThongKeDonHangRepository extends IHoaDonRepository {
    ...
    @Query(value = """
            SELECT COUNT(id) AS soLuong, SUM(tong_tien) - SUM(tien_tra_hang) AS tongTien FROM hoa_don
    ...
    @Query(value = """
        SELECT COUNT(id) AS soLuong, SUM(tong_tien) - SUM(tien_tra_hang) AS tongTien, DATE(created_at) AS ngayTao FROM hoa_don
        WHERE (trang_thai != 0 AND trang_thai != 6 AND trang_thai != 5 AND trang_thai != 9)
