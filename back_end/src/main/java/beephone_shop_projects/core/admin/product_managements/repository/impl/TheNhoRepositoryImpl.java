...
import beephone_shop_projects.core.admin.product_managements.model.request.FindTheNhoRequest;
...
public class TheNhoRepositoryImpl extends AbstractRepositoryImpl<TheNho, String> implements TheNhoRepository {
    ...
    public Page<TheNho> findAllTheNho(Pageable pageable, FindFilterProductsRequest findFilterProductsRequest) {
        Class<FindTheNhoRequest> findTheNhoRequest = FindTheNhoRequest.class;
        List<TheNho> theNhos = null;
        Long totalElements = 0L;
        try (EntityManager entityManager = this.entityManager) {
            CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();

            CriteriaQuery<TheNho> criteriaQuery = criteriaBuilder.createQuery(TheNho.class);
            CriteriaQuery<Long> countQuery = criteriaBuilder.createQuery(long.class);

            Root<TheNho> root = criteriaQuery.from(TheNho.class);
            Root<TheNho> countRoot = countQuery.from(TheNho.class);

            JpaPersistence<TheNho> configuration = new JpaPersistence<TheNho>(criteriaBuilder, criteriaQuery, countQuery, root, countRoot);
            ...
            Predicate searchPredicate = this.getPredicateContains(root, findTheNhoRequest, findFilterProductsRequest.getKeyword(), configuration);
            Predicate countSearchPredicate = this.getPredicateContains(countRoot, findTheNhoRequest, findFilterProductsRequest.getKeyword(), configuration);
            ...
            theNhos = this.buildQueryWithPaginationByPageableAndCriteriaQuery(pageable, configuration);
            totalElements = entityManager.createQuery(countQuery).getSingleResult();
        } catch (PersistenceException e) {
            logger.error(e.getMessage(), e);
        }
        return new PageImpl<>(theNhos, pageable, totalElements);
    }
}
