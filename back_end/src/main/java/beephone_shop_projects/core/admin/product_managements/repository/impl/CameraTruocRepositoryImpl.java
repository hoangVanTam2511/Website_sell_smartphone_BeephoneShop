...
public class CameraTruocRepositoryImpl extends AbstractRepositoryImpl<CameraTruoc, String> implements CameraTruocRepository {
    ...
    @Override
    public Page<CameraTruoc> findAllCameraTruoc(Pageable pageable, FindFilterCamerasRequest filterCamerasRequest) {
        Class<FindCameraRequest> findCameraTruocRequest = FindCameraRequest.class;
        List<CameraTruoc> cameraTruocs = null;
        Long totalElements = 0L;
        try (EntityManager entityManager = this.entityManager) {
            CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();

            CriteriaQuery<CameraTruoc> criteriaQuery = criteriaBuilder.createQuery(CameraTruoc.class);
            CriteriaQuery<Long> countQuery = criteriaBuilder.createQuery(long.class);

            Root<CameraTruoc> root = criteriaQuery.from(CameraTruoc.class);
            Root<CameraTruoc> countRoot = countQuery.from(CameraTruoc.class);

            JpaPersistence<CameraTruoc> configuration = new JpaPersistence<CameraTruoc>(criteriaBuilder, criteriaQuery, countQuery, root, countRoot);
            ...
            Predicate searchPredicate = this.getPredicateContains(root, findCameraTruocRequest, filterCamerasRequest.getKeyword(), configuration);
            Predicate countSearchPredicate = this.getPredicateContains(countRoot, findCameraTruocRequest, filterCamerasRequest.getKeyword(), configuration);
            ...
            cameraTruocs = this.buildQueryWithPaginationByPageableAndCriteriaQuery(pageable, configuration);
            totalElements = entityManager.createQuery(countQuery).getSingleResult();
        } catch (PersistenceException e) {
            logger.error(e.getMessage(), e);
        }
        return new PageImpl<>(cameraTruocs, pageable, totalElements);
    }
}
