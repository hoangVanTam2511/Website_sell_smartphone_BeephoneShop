...
const AddKhuyenMai = () => {
  ...
  //   ? listSanPham.filter((item) => item.trangThai === filterStatus)
  //   : listSanPham;
  ...
  const addKhuyenMai = () => {
    ...
    toast
      .promise(axios.post(apiURLKhuyenMai + "/add-khuyen-mai", obj), {
        success: {
          render({ data }) {
  ...
  const validationAll = () => {
    ...
    // if ((isIn(giaTriVoucher), 0)) {
    //   msg.formattedGiaTriVoucher = "Giá Trị Voucher phải lớn hơn 0 !!!";
    // }

    // if (isEmpty(giaTriVoucher)) {
    //   msg.formattedGiaTriVoucher = "Không để trống Giá Trị Voucher !!!";
    // }
