...
const BootstrapTable = () => {
  ...
  const loadUsers = async (page) => {
    const result = await axios.get(`http://localhost:8080/man-hinh/view-all?page=${page}`)
  ...
  const deleteProduct = async (id) => {
    Swal.fire({
      title: 'Bạn có muốn xóa kích thước màn hình này',
      showDenyButton: true,
  ...
  const deleteColor = async (id) => {
    await axios.delete(`http://localhost:8080/man-hinh/delete?id=${id}`)
    loadUsers()
  }
          ...
          <Card>
            <Card.Header className="d-flex justify-content-between">
              <div className="header-title">
                <h4 className="card-title">Quản lí màn hình
