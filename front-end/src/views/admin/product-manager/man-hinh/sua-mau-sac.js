...
const SuaMauSac = () => {
    ...
    const changeColor = async () => {
        await axios.put(`http://localhost:8080/man-hinh/update/${id}`, mauSac)
        navigate("/man-hinh")
    }

    const loadUser = async () => {
        const result = await axios.get(`http://localhost:8080/man-hinh/get-one/${id}`)
        setMauSac(result.data)
    }
    return (
        <div className='container' style={{ width: 600 }}>
            <h2 className='text-center'>Thay đổi màn hình</h2>
            <Form onSubmit={(e) => onSubmit(e)}>
                ...
                <Form.Group className="form-group">
                    <Form.Label htmlFor="pwd">Kích thước</Form.Label>
                    <Form.Control type="text"
