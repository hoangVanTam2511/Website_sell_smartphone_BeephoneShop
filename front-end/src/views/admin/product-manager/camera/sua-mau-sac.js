...
const SuaMauSac = () => {
    ...
    const onSubmit = async (e) => {
        ...
        Swal.fire({
            title: 'Bạn có muốn thay đổi camera này',
            showDenyButton: true,
    ...
    const changeColor = async () => {
        await axios.put(`http://localhost:8080/camera/update/${id}`, mauSac)
        navigate("/camera")
    }

    const loadUser = async () => {
        const result = await axios.get(`http://localhost:8080/camera/get-one/${id}`)
        setMauSac(result.data)
    }
    return (
        <div className='container' style={{ width: 600 }}>
            <h2 className='text-center'>Thay đổi màu sắc</h2>
            <Form onSubmit={(e) => onSubmit(e)}>
                ...
                <Form.Group className="form-group">
                    <Form.Label htmlFor="pwd">Tên camera</Form.Label>
                    <Form.Control type="text"
