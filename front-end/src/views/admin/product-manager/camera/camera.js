...
const HienThiKH = () => {
  ...
  const Delete = async(record) => {
    ...
    if(index > -1){
      Swal.fire({
        title: 'Bạn có muốn xóa camera này',
        showDenyButton: true,
      ...
      Swal.fire('Không tìm thấy camera', '', 'failed')
    }
  };
  ...
  const save = async (id) => {
    try {
      ...
      if (index > -1) {
        ...
        axios
          .put(`${apiURLCamera}/update/${id}`, updatedItem)
          .then((response) => {
  ...
  const columns = [
    ...
    {
      title: "Độ phân giải (MP)",
      dataIndex: "doPhanGiai",
      width: "15%",
      editable: true,
      ...getColumnSearchProps("Độ phân giải (MP)"),
    },
    {
      title: "Thao Tác",
      dataIndex: "operation",
    ...
    <>
      <div className="btn-add">
        ...
        <span className="bl-add">
          ...
          <Link to="/them-camera">
            <Button className="btn-them-tk">+ Thêm camera </Button>
          </Link>
