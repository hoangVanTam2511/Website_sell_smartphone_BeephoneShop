...
const HienThiKH = () => {
  ...
  const loadDatalistMauSac = (currentPage) => {
    ...
    axios.get(apiURLram + "/view-all?page=" + currentPage).then((response) => {
  ...
  const Delete = async(record) => {
    ...
    if(index > -1){
      ...
      Swal.fire('Không tìm thấy ram', '', 'failed')
    }
  };
  
  // delete
  const deleteColor = async (id) => {
    await axios.delete(`${apiURLram}/delete?id=${id}`).then(
      (response)=>{
        loadDatalistMauSac();
      })
  ...
  const save = async (id) => {
    try {
      ...
      if (index > -1) {
        ...
        axios
          .put(`${apiURLram}/update/${id}`, updatedItem)
          .then((response) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "15%",
      editable: true,
      ...getColumnSearchProps("Kích thước"),
    },
    {
      title: "Thao Tác",
      dataIndex: "operation",
    ...
    <>
      <div className="btn-add">
        ...
        <span className="bl-add">
          ...
          <Link to="/them-ram">
            <Button className="btn-them-tk">+ Thêm ram </Button>
          </Link>
