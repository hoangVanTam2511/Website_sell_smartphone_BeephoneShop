...
const HienThiKH = () => {
  ...
  const loadDatalistMauSac = (currentPage) => {
    ...
    axios.get(apiURLNhaSanXuat + "/view-all?page=" + currentPage).then((response) => {
  ...
  const Delete = async(record) => {
    ...
    if(index > -1){
      ...
      Swal.fire('Không tìm thấy nhà sản xuất', '', 'failed')
    }
  };
  
  // delete
  const deleteColor = async (id) => {
    await axios.delete(`${apiURLNhaSanXuat}/delete?id=${id}`).then(
      (response)=>{
        loadDatalistMauSac();
      })
  ...
  const save = async (id) => {
    try {
      ...
      if (index > -1) {
        ...
        axios
          .put(`${apiURLNhaSanXuat}/update/${id}`, updatedItem)
          .then((response) => {
      ...
      <div className="btn-add">
        ...
        <span className="bl-add">
          ...
          <Link to="/them-nha-san-xuat">
            <Button className="btn-them-tk">+ Thêm nhà sản xuất </Button>
          </Link>
