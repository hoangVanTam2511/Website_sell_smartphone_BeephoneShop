...
const SuaMauSac = () => {
    ...
    const changeColor = async () => {
        await axios.put(`http://localhost:8080/dong-san-pham/update/${id}`, mauSac)
        navigate("/mau-sac")
    }

    const loadUser = async () => {
        const result = await axios.get(`http://localhost:8080/dong-san-pham/get-one/${id}`)
        setMauSac(result.data)
    }
    return (
        <div className='container' style={{ width: 600 }}>
            <h2 className='text-center'>Thay đổi dòng sản phẩm</h2>
