...
const HienThiKH = () => {
  ...
  const loadDatalistMauSac = (currentPage) => {
    ...
    axios.get(apiURLDongSanPham + "/view-all?page=" + currentPage).then((response) => {
  ...
  const edit = (record) => {
    form.setFieldsValue({
      ma: record.ma,
      tenDongSanPham: record.tenDongSanPham,
    });
  ...
  const Delete = async(record) => {
    ...
    if(index > -1){
      Swal.fire({
        title: 'Bạn có muốn xóa dòng sản phẩm này',
        showDenyButton: true,
      ...
      Swal.fire('Không tìm thấy dòng sản phẩm', '', 'failed')
    }
  };
  
  // delete
  const deleteColor = async (id) => {
    await axios.delete(`${apiURLDongSanPham}/delete?id=${id}`).then(
      (response)=>{
        loadDatalistMauSac();
      })
  ...
  const save = async (id) => {
    try {
      ...
      if (index > -1) {
        ...
        axios
          .put(`${apiURLDongSanPham}/update/${id}`, updatedItem)
          .then((response) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "15%",
      editable: true,
      ...getColumnSearchProps("Tên dòng sản phẩm"),
    },
    {
      title: "Thao Tác",
      dataIndex: "operation",
    ...
    <>
      <div className="btn-add">
        ...
        <span className="bl-add">
          ...
          <Link to="/them-dong-san-pham">
            <Button className="btn-them-tk">+ Thêm dòng sản phẩm </Button>
          </Link>
