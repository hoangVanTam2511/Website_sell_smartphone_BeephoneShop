...
const BootstrapTable = () => {
  ...
  const loadUsers = async (page) => {
    const result = await axios.get(`http://localhost:8080/dong-san-pham/view-all?page=${page}`)
  ...
  const deleteColor = async (id) => {
    await axios.delete(`http://localhost:8080/dong-san-pham/delete?id=${id}`)
    loadUsers()
  }
          ...
          <Card>
            <Card.Header className="d-flex justify-content-between">
              <div className="header-title">
                <h4 className="card-title">Quản lí dòng sản phẩm
