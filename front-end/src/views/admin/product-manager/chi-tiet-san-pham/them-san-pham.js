...
import {
    CloseOutlined, ArrowRightOutlined, ArrowLeftOutlined, CheckOutlined, RadiusBottomleftOutlined,
...
const ThemSanPham = () => {
    ...
    const handleChange = (value) => {
        ...
        setchiTietSanPham({ ...chiTietSanPham, [String(target_value).slice(0, String(target_value).indexOf(":"))]: String(target_value).slice(String(target_value).indexOf(":") + 1) })
    };
    ...
    const handleChangeFormCauHinh = (value) => {
        const target_value = value.target.value
        setCauHinh({ ...cauHinh, [String(target_value).slice(0, String(target_value).indexOf(":"))]: String(target_value).slice(String(target_value).indexOf(":") + 1) })
    };
    ...
    const handleFileChangeImage = (event, location_image) => {
        ...
        if (file == null) return;
        const imageRef = ref(storage, `${chiTietSanPham.tenSanPham}/${location_image}/${file.name}`);
        uploadBytes(imageRef, file).then((res) => {
            listAll(ref(storage, chiTietSanPham.tenSanPham + "/" + location_image)).then(
                (responce) => {
    ...
    const clickPinImage = (event, location_image) => {
        ...
        setpinImage(map => new Map(map.set(location_image, true)))
        for (let [key, value] of pinImage) {
            if (key != location_image && (getIndexOfLocationImage(key) == getIndexOfLocationImage(location_image)))
...

                                                            {
                                                                sizeOfListSelected === 0 ?
                                                                    <>
                                                                        ...
                                                                        <br />
                                                                        <Button variant="contained" startIcon={<ArrowLeftOutlined />} style={{
                                                                        ...
                                                                        <Button variant="outlined" style={{ fontSize: 16, fontWeight: 600, marginLeft: `39%` }} onClick={showModalFormCauHinh}>
                                                                                        ...
                                                                                        <div style={{ display: 'flex', justifyContent: 'center', height: 100 }}>
                                                                                            ...
                                                                                            <div class="image-upload">
                                                                                                {urlImage.get(`image2${index}`) ?
                                                                                                    <FontAwesomeIcon style={{ color: `${pinImage.get(`image2${index}`) == true ? 'rgb(5 128 215)' : '#1b1b1b6b'}` }} className='icon_pin_image' icon={faThumbtack} onClick={(event) => clickPinImage(event, `image2${index}`)} />
                                                                                                    : <FontAwesomeIcon className='icon_pin_image' style={{ border: 0, color: 'white' }} icon={faThumbtack} />
                                                                                                }
                                                                                                <label for={id2}>
                                                                                                    <div class="upload-icon" style={{ backgroundImage: `url(${urlImage.get(`image2${index}`)})`, backgroundSize: 'cover' }}>
                                                                                                        {urlImage.get(`image2${index}`) ?
                                                                                                            <FontAwesomeIcon className='upload-icon-minus' icon={faMinus} onClick={(event) => confirmDelete(event, `image2${index}`)} />
                                                                                                            :
                                                                                                            <FontAwesomeIcon className='upload-icon-img' icon={faPlus} />
                                                                                                        }
                                                                                                    </div>
                                                                                                </label>
                                                                                                <input id={id2} type="file" onChange={(event) => handleFileChangeImage(event, `image2${index}`)} />
                                                                                            </div>
                                                                                            <div class="image-upload">
                                                                                                <form>
                                                                                                    {urlImage.get(`image3${index}`) ?
                                                                                                        <FontAwesomeIcon style={{ color: `${pinImage.get(`image3${index}`) == true ? 'rgb(5 128 215)' : '#1b1b1b6b'}` }} className='icon_pin_image' icon={faThumbtack} onClick={(event) => clickPinImage(event, `image3${index}`)} />
                                                                                                        : <FontAwesomeIcon className='icon_pin_image' style={{ border: 0, color: 'white' }} icon={faThumbtack} />
                                                                                                    }
                                                                                                    <label for={id3}>
                                                                                                        <div class="upload-icon" style={{ backgroundImage: `url(${urlImage.get(`image3${index}`)})`, backgroundSize: 'cover' }}>
                                                                                                            {urlImage.get(`image3${index}`) ?
                                                                                                                <FontAwesomeIcon className='upload-icon-minus' icon={faMinus} onClick={(event) => confirmDelete(event, `image3${index}`)} />
                                                                                                                :
                                                                                                                <FontAwesomeIcon className='upload-icon-img' icon={faPlus} />
                                                                                                            }
                                                                                                        </div>
                                                                                                    </label>
                                                                                                    <input id={id3} style={{ display: 'none' }} type="file" onChange={(event) => handleFileChangeImage(event, `image3${index}`)} />
                                                                                                </form>
                                                                                            </div>
                                                                                            <div class="image-upload">
                                                                                                {urlImage.get(`image4${index}`) ?
                                                                                                    <FontAwesomeIcon style={{ color: `${pinImage.get(`image4${index}`) == true ? 'rgb(5 128 215)' : '#1b1b1b6b'}` }} className='icon_pin_image' icon={faThumbtack} onClick={(event) => clickPinImage(event, `image4${index}`)} />
                                                                                                    : <FontAwesomeIcon className='icon_pin_image' style={{ border: 0, color: 'white' }} icon={faThumbtack} />
                                                                                                }
                                                                                                <label for={id4}>
                                                                                                    <div class="upload-icon" style={{ backgroundImage: `url(${urlImage.get(`image4${index}`)})`, backgroundSize: 'cover' }}>
                                                                                                        {urlImage.get(`image4${index}`) ?
                                                                                                            <FontAwesomeIcon className='upload-icon-minus' icon={faMinus} onClick={(event) => confirmDelete(event, `image4${index}`)} />
                                                                                                            :
                                                                                                            <FontAwesomeIcon className='upload-icon-img' icon={faPlus} />
                                                                                                ...
                                                                                                <input id={id4} type="file" onChange={(event) => handleFileChangeImage(event, `image4${index}`)} />
                                                                                            </div>
                                                                                            <div class="image-upload">
                                                                                                {urlImage.get(`image5${index}`) ?
                                                                                                    <FontAwesomeIcon style={{ color: `${pinImage.get(`image5${index}`) == true ? 'rgb(5 128 215)' : '#1b1b1b6b'}` }} className='icon_pin_image' icon={faThumbtack} onClick={(event) => clickPinImage(event, `image5${index}`)} />
                                                                                                    : <FontAwesomeIcon className='icon_pin_image' style={{ border: 0, color: 'white' }} icon={faThumbtack} />
                                                                                                }
                                                                                                <label for={id5}>
                                                                                                    <div class="upload-icon" style={{ backgroundImage: `url(${urlImage.get(`image5${index}`)})`, backgroundSize: 'cover' }}>
                                                                                                        {urlImage.get(`image5${index}`) ?
                                                                                                            <FontAwesomeIcon className='upload-icon-minus' icon={faMinus} onClick={(event) => confirmDelete(event, `image5${index}`)} />
                                                                                                            :
                                                                                                            <FontAwesomeIcon className='upload-icon-img' icon={faPlus} />
                                                                                                        }
                                                                                                    </div>
                                                                                                </label>
                                                                                                <input id={id5} type="file" onChange={(event) => handleFileChangeImage(event, `image5${index}`)} />
                                                                                            </div>
