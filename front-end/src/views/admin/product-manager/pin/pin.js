...
import { apiURLPin } from "../../../../service/api";
...
const HienThiKH = () => {
  ...
  const loadDatalistMauSac = (currentPage) => {
    ...
    axios.get(apiURLPin + "/view-all?page=" + currentPage).then((response) => {
  ...
  const Delete = async(record) => {
    ...
    if(index > -1){
      ...
      Swal.fire('Không tìm thấy pin', '', 'failed')
    }
  };
  
  // delete
  const deleteColor = async (id) => {
    await axios.delete(`${apiURLPin}/delete?id=${id}`).then(
      (response)=>{
        loadDatalistMauSac();
      })
  ...
  const save = async (id) => {
    try {
      ...
      if (index > -1) {
        ...
        axios
          .put(`${apiURLPin}/update/${id}`, updatedItem)
          .then((response) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "15%",
      editable: true,
      ...getColumnSearchProps("Dung luợng"),
    },
    {
      title: "Thao Tác",
      dataIndex: "operation",
    ...
    <>
      <div className="btn-add">
        ...
        <span className="bl-add">
          ...
          <Link to="/them-pin">
            <Button className="btn-them-tk">+ Thêm pin </Button>
          </Link>
