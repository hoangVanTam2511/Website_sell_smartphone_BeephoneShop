...
const HienThiKH = () => {
  ...
  const loadDatalistMauSac = (currentPage) => {
    ...
    axios.get(apiURLChip + "/view-all?page=" + currentPage).then((response) => {
  ...
  const Delete = async(record) => {
    ...
    if(index > -1){
      ...
      Swal.fire('Không tìm thấy chip', '', 'failed')
    }
  };
  
  // delete
  const deleteColor = async (id) => {
    await axios.delete(`${apiURLChip}/delete?id=${id}`).then(
      (response)=>{
        loadDatalistMauSac();
      })
  ...
  const save = async (id) => {
    try {
      ...
      if (index > -1) {
        ...
        axios
          .put(`${apiURLChip}/update/${id}`, updatedItem)
          .then((response) => {
    ...
    <>
      <div className="btn-add">
        ...
        <span className="bl-add">
          ...
          <Link to="/them-chip">
            <Button className="btn-them-tk">+ Thêm chip </Button>
          </Link>
