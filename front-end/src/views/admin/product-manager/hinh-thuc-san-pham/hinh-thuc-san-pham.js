...
import { apiURLHinhThucSanPham } from "../../../../service/api";
...
const HienThiKH = () => {
  ...
  const loadDatalistMauSac = (currentPage) => {
    ...
    axios.get(apiURLHinhThucSanPham + "/view-all?page=" + currentPage).then((response) => {
  ...
  const Delete = async(record) => {
    ...
    if(index > -1){
      Swal.fire({
        title: 'Bạn có muốn xóa hình thức sản phẩm này',
        showDenyButton: true,
      ...
      Swal.fire('Không tìm thấy hình thức sản phẩm', '', 'failed')
    }
  };
  
  // delete
  const deleteColor = async (id) => {
    await axios.delete(`${apiURLHinhThucSanPham}/delete?id=${id}`).then(
      (response)=>{
        loadDatalistMauSac();
      })
  ...
  const save = async (id) => {
    try {
      ...
      if (index > -1) {
        ...
        axios
          .put(`${apiURLHinhThucSanPham}/update/${id}`, updatedItem)
          .then((response) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "15%",
      editable: true,
      ...getColumnSearchProps("Hình thức sản phẩm"),
    },
    {
      title: "Thao Tác",
      dataIndex: "operation",
      ...
      <div className="btn-add">
        <span>
          <Form style={{ width: "20em", display: "inline-block" }}>
            <h2>Quản lí hình thức sản phẩm</h2>
          </Form>
        </span>
        ...
        <span className="bl-add">
          ...
          <Link to="/them-hinh-thuc-san-pham">
            <Button className="btn-them-tk">+ Thêm hình thức sản phẩm </Button>
          </Link>
