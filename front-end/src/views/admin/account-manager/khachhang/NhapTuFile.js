  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (e) => {
      ...
      for(var i = 2;i < parsedData.length;i++){
        ...
        if(ma === '' || ten === '' || email === '' || sdt === '' || gioiTinh === '' || ma === null || ten === null || email === null || sdt === null || gioiTinh === null){ 
          handleOpenAlertVariant("Import file thất bại.Vui lòng không được để trống các trường.", Notistack.SUCCESS);       
          return;
        }

        if(ma.trim() === '' || ten.trim() === '' || email.trim() === '' || sdt.trim() === '' || gioiTinh.trim() === ''){
          handleOpenAlertVariant("Import file thất bại.Vui lòng không được để trống các trường", Notistack.ERROR);
          return;
        }

        if(handleHoVaTenKH(ten) === false){
          handleOpenAlertVariant("Import file thất bại.Vui lòng kiểm tra lại cột họ tên", Notistack.ERROR);
          return;
        }

        if(handleEmailChange(email) === false){
          handleOpenAlertVariant("Import file thất bại.Vui lòng kiểm tra lại cột email", Notistack.ERROR);
          return;
        }

        if(handleSDT(sdt) === false){
          handleOpenAlertVariant("Import file thất bại.Vui lòng kiểm tra lại cột số điện thoại", Notistack.ERROR);
          return;
        }

        allCustomers.forEach((e) => {
          if( String(e.ma) === String(ma) && String(e.email) === String(email) && String(e.sdt) === String(sdt) && String(e.ten) === String(ten)){
            flag = true;
          }
        })
            ...
            <Button className="custom-button1" style={{ height: "44px" }}>
              ...
              />
               <input 
                type="file" 
                accept=".xlsx, .xls" 
                style={{ opacity:0, position: 'absolute', height: "40px", top: '0px', left: '0px', width: '123px' }}
