...
const TabItem = ({
    ...
    await axios
      ...
      .then(async (response) => {
        ...
        const province = provinces.find(
          (item) => item.ProvinceName === address.tinhThanhPho
        );
        setSelectedProvince(province.ProvinceID);

        const district = districts.find(
          (item) => item.DistrictName === address.quanHuyen
        );
        setSelectedDistrict(district.DistrictID);

        await getAllWardGhnByIdDistrict(
          district.DistrictID,
          true,
          address.xaPhuong
        );
  ...
  const getAllWardGhnByIdDistrict = async (
    districtId,
    selectWard,
    district
  ) => {
    await axios
      ...
      .then((response) => {
        ...
        if (selectWard) {
          const ward = response.data.data.find(
            (item) => item.WardName === district
          );
  ...
  const columns = [
            ...
            <div className="product-img">
              <img
                src={
                  item &&
                  item.sanPhamChiTiet &&
                  item.sanPhamChiTiet.images &&
                  item.sanPhamChiTiet.images[0].duongDan
                }
                class=""
                alt=""
            ...
            <div classNamountme="product-name">
              ...
              >
                {item.sanPhamChiTiet.sanPham.tenSanPham +
                  "\u00A0" +
                  item.sanPhamChiTiet.cauHinh.ram.kichThuoc +
                  "/" +
                  item.sanPhamChiTiet.cauHinh.rom.kichThuoc +
                  "GB" +
                  " " +
                  `(${item.sanPhamChiTiet.cauHinh.mauSac.tenMauSac})`}
              </span>
            </div>
            <div className="">
              <span
    ...
    },
    {
      ...
      width: "1%",
      render: (text, item) => (
        <InputNumberAmountCart
          id={item.id}
          amountCurrent={item.soLuong}
          update={updateAmount}
        />
      ),
    },
    ...
    <>
      <div style={{ width: "", height: "auto" }} className="mt-2">
        <div
          className={"p-2"}
          style={{
            boxShadow: "0 0.1rem 0.3rem #00000050",
            marginTop: "",
            height: "auto",
          }}
        >
          ...
          <div>
            ...
            {cartItems && cartItems.length == 1 ? (
              <div className="" style={{ height: "320.5px" }}></div>
            ) : cartItems && cartItems.length === 2 ? (
              <div className="" style={{ height: "173px" }}></div>
            ) : cartItems && cartItems.length > 2 ? (
                ...
                <div className="mt-4">
                  {delivery ? (
                    <div style={{ width: "99.5%" }} className="">
                      <div>
                        <TextFieldName
                          nameDefault={customerName}
                          getName={getName}
                        />
                      </div>
                      <div>
                        <TextFieldPhone
                          phoneDefault={customerPhone}
                          getPhone={getPhone}
                        />
                      </div>
                      ...
                      >
                        ...
                        selectedWard != "" ? (
                          <>
                            <div className="d-flex">
                              <img
                                src="https://www.svgrepo.com/show/236705/delivery-truck-truck.svg"
                              ...
                              <div className="mt-2 pt-1">
                                ...
                                >
                                  Thời gian giao hàng dự kiến: {receiveDate}
                                </span>
                              </div>
                              <img
