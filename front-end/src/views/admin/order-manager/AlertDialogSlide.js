...
export function ProductsDialog(props) {
  ...
  const [selectedValueCategorys, setSelectedValueCategorys] = React.useState([0]);
  const [selectedValueBrands, setSelectedValueBrands] = React.useState([0]);
  const [selectedValueOperas, setSelectedValueOperas] = React.useState(["None"]);
  ...
  const findProductsByMultipleCriteriaWithPagination = (page) => {
    ...
    params.append('danhMucs', selectedValueCategorys.length === 1 && selectedValueCategorys[0] === 0 ? [] : selectedValueCategorys);
    params.append('hangs', selectedValueBrands.length === 1 && selectedValueBrands[0] === 0 ? [] : selectedValueBrands);
    params.append('heDieuHanhs', selectedValueOperas.length === 1 && selectedValueOperas[0] === "None" ? operas : selectedValueOperas);
    params.append('chips', selectedValueCpus.length === 1 && selectedValueCpus[0] === 0 ? [] : selectedValueCpus);
    params.append('manHinhs', selectedValueScreens.length === 1 && selectedValueScreens[0] === 0 ? [] : selectedValueScreens);
    params.append('pins', selectedValuePins.length === 1 && selectedValuePins[0] === 0 ? [] : selectedValuePins);
    params.append('rams', selectedValueRams.length === 1 && selectedValueRams[0] === 0 ? [] : selectedValueRams);
    params.append('roms', selectedValueRoms.length === 1 && selectedValueRoms[0] === 0 ? [] : selectedValueRoms);
    params.append('fromPrice', fromPrice || 0);
    params.append('toPrice', toPrice || 51900000);
    axios
      .get(`http://localhost:8080/api/products/product-items/page?${params}`, {
      })
      .then((response) => {
  ...
  }, [currentPage, keyword, size, selectedValueRams, selectedValuePins, selectedValueRoms, selectedValueScreens, selectedValueCpus, selectedValueBrands, selectedValueOperas, selectedValueCategorys, fromPrice, toPrice])
  ...
  const TableProduct = () => {
    return (
      <>
        <div className="">
          <TableAntd className="table-container "
            columns={columns}
            rowKey="ma"
            dataSource={products}
                ...
                >
                  ...
                  <FormControl sx={{ maxWidth: 200 }} size="small">
                    <SelectMui
                      ...
                      renderValue={(selected) => selected && selected.length > 1 ? selected
                        .filter(id => categorys.find(c => c.id === id)) // Loại bỏ các giá trị không hợp lệ
                        .map(id => categorys.find(c => c.id === id).tenDanhMuc) // Lấy tên danh mục tương ứng
                        .join(', ') : 'Chọn Danh Mục'}
                    >
                      {categorys.map((c) => (
                        <MenuItem key={c.id} value={c.id}>
                          <CheckBoxMui checked={selectedValueCategorys.indexOf(c.id) > -1} />
                  ...
                  >
                    <SelectMui
                      ...
                      renderValue={(selected) => selected && selected.length > 1 ? selected
                        ...
                        .map(id => listHang.find(c => c.id === id).tenHang) // Lấy tên danh mục tương ứng
                        .join(', ') : 'Chọn Hãng'}
                    >
                      {listHang.map((c) => (
                        <MenuItem key={c.id} value={c.id}>
                          <CheckBoxMui checked={selectedValueBrands.indexOf(c.id) > -1} />
                    ...
                    <SelectMui
                      ...
                      onChange={(e) => {
                        if (e.target.value.length === 0) {
                          ...
                          const values = e.target.value.filter((value) => value !== "None");
                          setSelectedValueOperas(values);
                        }
                      }}
                      renderValue={(selected) => selected && selected.filter((value) => value !== "None").length > 0 ? selected
                        .filter(id => ["ANDROID", "IOS"].find(c => c === id)) // Loại bỏ các giá trị không hợp lệ
                        .map(id => ["ANDROID", "IOS"].find(c => c === id)) // Lấy tên danh mục tương ứng
                        .join(', ') : 'Chọn Hệ Điều Hành'}
                    >
                      {["ANDROID", "IOS"].map((c) => (
                        <MenuItem key={c} value={c}>
                          <CheckBoxMui checked={selectedValueOperas.indexOf(c) > -1} />
                  ...
                  >
                    <SelectMui
                      ...
                      renderValue={(selected) => selected && selected.length > 1 ? selected
                        ...
                        .map(id => listChip.find(c => c.id === id).tenChip) // Lấy tên danh mục tương ứng
                        .join(', ') : 'Chọn Chip'}
                    >
                      {listChip.map((c) => (
                        <MenuItem key={c.id} value={c.id}>
                          <CheckBoxMui checked={selectedValueCpus.indexOf(c.id) > -1} />
                  ...
                  >
                    <SelectMui
                      ...
                      renderValue={(selected) => selected && selected.length > 1 ? selected
                        ...
                        .map(id => listRam.find(c => c.id === id).dungLuong + "GB") // Lấy tên danh mục tương ứng
                        .join(', ') : 'Chọn Ram'}
                    >
                      {listRam.map((c) => (
                        <MenuItem key={c.id} value={c.id}>
                          <CheckBoxMui checked={selectedValueRams.indexOf(c.id) > -1} />
                  ...
                  >
                    <SelectMui
                      ...
                      renderValue={(selected) => selected && selected.length > 1 ? selected
                        ...
                        .map(id => listRom.find(c => c.id === id).dungLuong === 1024 ? 1 + "TB" : listRom.find(c => c.id === id).dungLuong + "GB") // Lấy tên danh mục tương ứng
                        .join(', ') : 'Chọn Rom'}
                    >
                      {listRom.map((c) => (
                        <MenuItem key={c.id} value={c.id}>
                          <CheckBoxMui checked={selectedValueRoms.indexOf(c.id) > -1} />
                          <ListItemText primary={c.dungLuong === 1024 ? 1 + "TB" : c.dungLuong + "GB"} />
                        </MenuItem>
                      ))}
                    </SelectMui>
                  </FormControl>
                  ...
                  >
                    <SelectMui
                      ...
                      renderValue={(selected) => selected && selected.length > 1 ? selected
                        ...
                        .map(id => listManHinh.find(c => c.id === id).loaiManHinh + " " + `(${listManHinh.find(c => c.id === id).doPhanGiaiManHinh.chieuRong + " x " + listManHinh.find(c => c.id === id).doPhanGiaiManHinh.chieuDai} pixels) ` + listManHinh.find(c => c.id === id).kichThuoc + `"`) // Lấy tên danh mục tương ứng
                        .join(', ') : 'Chọn Màn Hình'}
                    >
                      {listManHinh.map((c) => (
                        <MenuItem key={c.id} value={c.id}>
                          <CheckBoxMui checked={selectedValueScreens.indexOf(c.id) > -1} />
                          <ListItemText primary={c.loaiManHinh + " " + `(${c.doPhanGiaiManHinh.chieuRong + " x " + c.doPhanGiaiManHinh.chieuDai} pixels) ` + c.kichThuoc + `"`} />
                        </MenuItem>
                      ))}
                    </SelectMui>
                  </FormControl>
                  ...
                  >
                    <SelectMui
                      ...
                      renderValue={(selected) => selected && selected.length > 1 ? selected
                        ...
                        .map(id => listPin.find(c => c.id === id).loaiPin + " " + listPin.find(c => c.id === id).dungLuong + " mAh") // Lấy tên danh mục tương ứng
                        .join(', ') : 'Chọn Pin'}
                    >
                      {listPin.map((c) => (
                        <MenuItem key={c.id} value={c.id}>
                          <CheckBoxMui checked={selectedValuePins.indexOf(c.id) > -1} />
                          <ListItemText primary={c.loaiPin + " " + c.dungLuong + " mAh"} />
                        </MenuItem>
                      ))}
                    </SelectMui>
                  </FormControl>
